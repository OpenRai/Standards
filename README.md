# OpenRai Initiative Standards

A collection of technical specifications designed to promote interoperability across the wider [Nano](https://nano.org/) ecosystem.

## Standards Index

<!-- ORIS-INDEX-START -->
- [ORIS-001: Nano Off-chain Message Signing (NOMS)](rfcs/ORIS-001.md) (Implementation Draft)
- [ORIS-002: NanoNyms: Privacy-Enhancing Reusable Payment Codes](rfcs/ORIS-002.md) (Implementation Draft)
- [ORIS-003: NanoNyms Payment Event Schema](rfcs/ORIS-003.md) (Working Draft)
- [ORIS-004: NanoNyms Nostr Transport Profile](rfcs/ORIS-004.md) (Working Draft)
- [ORIS-005: NanoNyms Profile for x402 Exact Payments](rfcs/ORIS-005.md) (Working Draft)
- [ORIS-006: Nano CAIP Identifiers](rfcs/ORIS-006.md) (Implementation Draft)
- [ORIS-007: Nano Application-Level Metadata, Correlation, and Signaling Patterns](rfcs/ORIS-007.md) (Working Draft)
- [ORIS-008: Reliable Nano Payment Integration](rfcs/ORIS-008.md) (Working Draft)
- [ORIS-009: Nano Payment Targets for `payto:`](rfcs/ORIS-009.md) (Working Draft)
- [ORIS-010: API-Key Authentication for Nano RPC](rfcs/ORIS-010.md) (Working Draft)
- [ORIS-011: Nano App-Intent URI Profiles](rfcs/ORIS-011.md) (Working Draft)
- [ORIS-012: The `rai` Sub-Unit and Safe Amount Serialization for Nano Applications](rfcs/ORIS-012.md) (Working Draft)
<!-- ORIS-INDEX-END -->

## Document Statuses

- **Working Draft:** A reviewable proposal that can still change as implementations and feedback develop.
- **Implementation Draft:** A proposal with at least one working implementation. Details can still change before the document becomes stable.
- **Stable:** A specification considered suitable for broad interoperability. Changes should preserve compatibility.
- **Informational:** Guidance or background material that does not define an interoperability contract.
